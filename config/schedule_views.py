...
from symposion.sponsorship.models import Benefit, Sponsor, SponsorBenefit, SponsorLevel
...
def sponsor_add(request):
    ...
    return render_to_response(
        "symposion/sponsorship/add.html",
        {"form": form},
        context_instance=RequestContext(request),
    )
...
def sponsor_detail(request, pk):
    ...
    return render(
        request,
        "symposion/sponsorship/detail.html",
        {"sponsor": sponsor, "form": form, "formset": formset},
    )
...
def sponsor_export_data(request):
    ...
    for level, level_sponsors in itertools.groupby(
        ...
        for sponsor, next in pairwise(level_sponsors):
            ...
            description = (
                description if description else "-- NO DESCRIPTION FOR THIS SPONSOR --"
            )
        ...
        for benefit in benefits:
            ...
            for level in SponsorLevel.objects.all():
                ...
                for sponsor in Sponsor.objects.filter(level=level, active=True):
                    sponsor_name = (
                        sponsor.name.lower().replace(" ", "_").replace("/", "_")
                    )
                    ...
                    for sponsor_benefit in SponsorBenefit.objects.filter(
                        ...
                        if os.path.exists(sponsor_benefit.upload.path):
                            ...
                            with open(sponsor_benefit.upload.path, "rb") as f:
                                ...
                                zipinfo = ZipInfo(
                                    filename=full_dir + "/" + fname, date_time=modtime
                                )
                                zipfile.writestr(zipinfo, f.read())
                        else:
                            log.debug(
                                "No such sponsor file: %s" % sponsor_benefit.upload.path
                            )
    finally:
        zipfile.close()

    response = HttpResponse(zip_stringio.getvalue(), content_type="application/zip")
