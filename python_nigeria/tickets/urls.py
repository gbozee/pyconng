...
urlpatterns = [
    ...
    url(r'^create/(?P<order>[\w.@+-]+)/$', login_required(views.CreateTicketview.as_view()), name="create_ticket"),
    url(r'^sales/(?P<pk>\d+)/$', login_required(views.SalesTicketView.as_view()), name="sales"),
    ...
    url(r'^paystack/validate/(?P<order>[\w.@+-]+)/(?P<code>[\w.@+-]+)$',
        view=login_required(views.validate_paystack_ref), name='validate_paystack'),
